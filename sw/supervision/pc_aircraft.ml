...
let ac_combo_handler = fun gui (ac_combo:Gtk_tools.combo) target_combo flash_combo (log:string->unit) ->
  ...
  *)
  ...
  (* Checkout *)
  let callback = fun _ ->
    match GToolbox.question_box ~title:"Checkout code of last successfull flight" ~default:2 ~buttons:["Yes"; "Cancel"] "Are you sure you have no uncommited changes?" with
    | 1 -> (checkout_git_version gui#label_release#text )
    | _ -> ()
  in
  ignore (gui#button_checkout_release#connect#clicked ~callback);
